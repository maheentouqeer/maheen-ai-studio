...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    tl.from(".hero-title", {
      ...
      .from(".hero-description", { opacity: 0, y: 20, duration: 0.8, ease: "power3.out" }, "-=0.4")
      .from(".hero-buttons", { opacity: 0, y: 20, duration: 0.8, ease: "power3.out" }, "-=0.3")
      .from("[data-depth]", { opacity: 0, scale: 0.8, duration: 1, stagger: 0.1, ease: "back.out(1.7)" }, "-=0.6");
    ...
    const onMove = (e: MouseEvent) => {
      ...
      el.querySelectorAll<HTMLElement>("[data-depth]").forEach((layer) => {
        ...
        gsap.to(layer, { x: tx, y: ty, duration: 0.3, ease: "power2.out" });
      });
    };
  ...
  return (
    <section id="home" ref={sectionRef as any} className="relative overflow-hidden">
      ...
      <div className="container py-24 md:py-32">
        <div className="hero-title text-5xl md:text-7xl lg:text-8xl xl:text-9xl mb-6">
          <TextPressure
            text={about.name}
            className="hero-name"
            minFontWeight={400}
        ...
        <div className="hero-buttons mt-10 flex flex-col sm:flex-row gap-4">
          ...
          <a href="#contact">
            <Button
              variant="outline"
              size="lg"
              className="glass-panel hover-scale text-lg px-8 py-6 rounded-xl font-semibold border-primary/30 hover:border-primary/60"
            >
              Let's Connect
            </Button>
          </a>
